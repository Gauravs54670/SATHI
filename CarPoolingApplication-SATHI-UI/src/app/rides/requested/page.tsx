...
export default function MyRideRequestsPage() {
  ...
  const rideStatusColors: Record<string, string> = {
    ...
    RIDE_CANCELLED: "bg-rose-500/10 text-rose-500 border-rose-500/20",
  };
          ...
          <div className="space-y-8 animate-fade-in-up-delay">
            {requests.map((req, index) => (
              ...
              >
                   ...
                   {acceptedDrivers[req.rideRequestedId] && (
                     <div className="mt-3 p-3 rounded-2xl bg-white/[0.03] border border-white/5 space-y-2">
                       <div className="flex items-center gap-2">
                         ...
                         <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{acceptedDrivers[req.rideRequestedId].vehicleModel}</span>
                       </div>
                       <div className="flex items-center gap-2">
                         <svg className="w-4 h-4 text-slate-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
                         </svg>
                         <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">{acceptedDrivers[req.rideRequestedId].vehicleNumber}</span>
                       </div>
                     </div>
                   )}
                </div>
                ...
                <div className="p-8 md:w-1/3 flex flex-col justify-center items-center gap-4 bg-white/[0.01] border-t md:border-t-0 md:border-l border-white/5">
                    ...
                    <div className="flex flex-col gap-3 pt-6 border-t border-white/5 w-full">
                        {req.rideRequestStatus === 'ACCEPTED' && acceptedDrivers[req.rideRequestedId] ? (
                            <>
                                <button 
                                    onClick={() => router.push(`/ride/track/${req.rideRequestedId}`)}
                                    className="w-full px-4 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-[10px] font-black uppercase tracking-widest hover:shadow-lg hover:shadow-indigo-500/20 transition-all active:scale-95 flex items-center justify-center gap-2"
                                >
                                    <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                    </svg>
                                    <span>Track Ride & OTP</span>
                                </button>
                                <button 
                                    onClick={() => window.open(`sms:${acceptedDrivers[req.rideRequestedId].driverPhoneNumber}`)}
                                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-indigo-400 text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2"
                                >
                                    <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <button 
                                    ...
                                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-emerald-400 text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2"
                                >
                                    <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
